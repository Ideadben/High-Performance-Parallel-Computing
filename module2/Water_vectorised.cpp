...
class System {
public:
    Molecules molecules;          // all the types of molecules in the system (In this case just one type, water)
...
void BuildNeighborList(System& sys){
    std::vector<double> distances2(sys.molecules.no_mol); // array of distances to other molecules
    std::vector<size_t> index(sys.molecules.no_mol);      // index array used for argsort
    for(size_t i = 0; i < sys.molecules.no_mol; i++){     // For each molecule, build the neighbour list
...
void UpdateBondForces(System& sys){
    Molecules& molecule = sys.molecules; // Loop over all molecules in the system
...
void UpdateNonBondedForces(System& sys){
    ...
    for (size_t ai = 0; ai < sys.molecules.atoms.size(); ai++)
    for (size_t aj = 0; aj < sys.molecules.atoms.size(); aj++){ // iterate over all pairs of atoms, similar as well as dissimilar
        auto& atom1 = sys.molecules.atoms[ai];
        auto& atom2 = sys.molecules.atoms[aj];
...
System MakeWater(size_t N_molecules){
    ...
    Molecules molecules = Molecules(atoms, waterbonds, waterangle, N_molecules);
